...
load("../data/CombinedUF35minSalVAMPH_input.RData")
...
# writing data
write_csv(data.frame(genes_filt, counts_filt), "../output/CombinedUF35min_RNA/raw_counts_filt.csv")
write_csv(data.frame(genes_filt, rpkm_filt), "../output/CombinedUF35min_RNA/rpkm_counts_filt.csv")
write_csv(data.frame(genes_filt, tpm_filt),  "../output/CombinedUF35min_RNA/tpm_counts_filt.csv")
```
...
dds<-DESeqDataSetFromMatrix(countData = d$counts, 
                            colData = Samples_QC,
                            design =  ~ Treatment + pbc + Replicate)
dds<-DESeq(dds)
```
...
write.csv(exp_mat, "../output/CombinedUF35min_RNA/VarianceStabilizedCounts.csv")
head(exp_mat)
```
...
```{r, echo = F}
#-saving plots
png("../output/CombinedUF35min_RNA/Tree_vstCountss.png", height = 7, width = 12, units = 'in', res = 300)
plot(fit)
dev.off()

png("../output/CombinedUF35min_RNA/PCA.png", height = 7, width = 12, units = 'in', res = 300)
pcaPlt
dev.off()
```
...
exp_mat = exp_mat %>% 
  mutate(amphUFVsalUF_padj = amphUFvsalUF$padj,
         amphUFVsalUF_lfc = amphUFvsalUF$log2FoldChange,
         amphUFvsalUF_sig = case_when( amphUFVsalUF_lfc > lfc_c & amphUFVsalUF_padj < p_c ~ "UP",
                                       amphUFVsalUF_lfc < lfc_c & amphUFVsalUF_padj < p_c ~ "DOWN",
                                       amphUFVsalUF_padj > p_c ~ "not sig",
                                       is.na(amphUFVsalUF_padj) ~ "not sig"))
...
```{r}
write_csv(exp_mat, "../output/CombinedUF35min_RNA/diffExpressionMatrix.csv")
write_csv(as.data.frame(amphUFvsalUF@listData), "../output/CombinedUF35min_RNA/amphUFvsalUF.csv")
```

Volcano plots
```{r figure 2F, fig.height=9, fig.width=14}
gene_text_size = 7
axis_text_size = 22
title_size = 28


volcano_p<-as.data.frame(amphUFvsalUF) %>%
...
ma_p <-as.data.frame(amphUFvsalUF) %>%
...
png( "../output/CombinedUF35min_RNA/volcano.png",height = 10, width = 20, units = 'in', res = 300)
volcano_p
dev.off()

png( "../output/CombinedUF35min_RNA//MA.png", height = 10, width = 20, units = 'in', res = 300, bg = "transparent")
ma_p
dev.off()
```
...
```{r}
geneOfInterest = "Sst"
plotCounts(dds, gene =which(genes_filt %in% geneOfInterest), intgroup = "samples" , col = Samples_QC$Replicate)
```


```{r, echo = F,eval = F}
#saving file and data
save.image("../output/CombinedUF35min_RNA/CombinedUF35minSalVAMPH_output.RData")
```

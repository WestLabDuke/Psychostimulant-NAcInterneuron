...
keep <- which(rowSums(countCheck) >= 9)
...
# writing data
write_csv(data.frame(genes_filt, counts_filt), "../output/PVIPvUF_RNA/raw_counts_filt.csv")
write_csv(data.frame(genes_filt, rpkm_filt), "../output/PVIPvUF_RNA/rpkm_counts_filt.csv")
write_csv(data.frame(genes_filt, tpm_filt),  "../output/PVIPvUF_RNA/tpm_counts_filt.csv")
```
...
write.csv(exp_mat, "../output/PVIPvUF_RNA/VarianceStabilizedCounts.csv")
head(exp_mat)
```
...
```{r, echo = F}
png("../output/PVIPvUF_RNA/Tree_vstCounts.png", height = 7, width = 12, units = 'in', res = 300)
plot(fit)
dev.off()

png("../output/PVIPvUF_RNA/PCA.png", height = 7, width = 12, units = 'in', res = 300)
pcaPlt
dev.off()
```
...
exp_mat = exp_mat %>% 
  mutate(salIPvUF_padj = salIPvUF$padj,
         salIPvUF_lfc = salIPvUF$log2FoldChange,
         salIPvUF_sig = case_when( salIPvUF_lfc > lfc_c & salIPvUF_padj < p_c ~ "UP",
                                       salIPvUF_lfc < lfc_c & salIPvUF_padj < p_c ~ "DOWN",
                                       salIPvUF_padj > p_c ~ "not sig",
                                       is.na(salIPvUF_padj) ~ "not sig"))
...
```{r}
write_csv(exp_mat, "../output/PVIPvUF_RNA/diffExpressionMatrix.csv")
write_csv(as.data.frame(salIPvUF@listData), "../output/PVIPvUF_RNA/amphIPvsalIP.csv")
```


Volcano plots
```{r Figure1G, fig.height=9, fig.width=14}
gene_text_size = 7
axis_text_size = 22
title_size = 28

volcano_p <-as.data.frame(salIPvUF) %>%
...
png( "../output/PVIPvUF_RNA/volcano.png",height = 10, width = 20, units = 'in', res = 300)
volcano_p
dev.off()

png( "../output/PVIPvUF_RNA//MA.png", height = 10, width = 20, units = 'in', res = 300, bg = "transparent")
ma_p
dev.off()
```
...
save.image("../output/PVIPvUF_RNA/PVIPvUF_RNA_output.RData")
```
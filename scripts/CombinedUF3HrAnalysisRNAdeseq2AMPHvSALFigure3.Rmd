...
load("../data/CombinedUF3HrSalVAMPH_input.RData")
...
# writing data
write_csv(data.frame(genes_filt, counts_filt), "../output/CombinedUF3hr_RNA/raw_counts_filt.csv")
write_csv(data.frame(genes_filt, rpkm_filt), "../output/CombinedUF3hr_RNA/rpkm_counts_filt.csv")
write_csv(data.frame(genes_filt, tpm_filt),  "../output/CombinedUF3hr_RNA/tpm_counts_filt.csv")
```
...
write.csv(exp_mat, "../output/CombinedUF3hr_RNA/VarianceStabilizedCounts.csv")
head(exp_mat)
```
...
```{r}
#-saving plots
png("../output/CombinedUF3hr_RNA/Tree_vstCounts.png", height = 7, width = 12, units = 'in', res = 300)
plot(fit)
dev.off()

png("../output/CombinedUF3hr_RNA/PCA.png", height = 7, width = 12, units = 'in', res = 300)
pcaPlt
dev.off()
```
...
```{r}
write_csv(exp_mat, "../output/CombinedUF3hr_RNA/diffExpressionMatrix.csv")
write_csv(as.data.frame(amphUFvsalUF@listData), "../output/CombinedUF3hr_RNA/amphUFvsalUF.csv")
```

Volcano plots
```{r figure 3F, fig.height=9, fig.width=14}
gene_text_size = 7
axis_text_size = 22
title_size = 28
...
png( "../output/CombinedUF3hr_RNA/volcano.png",height = 10, width = 20, units = 'in', res = 300)
volcano_p
dev.off()

png( "../output/CombinedUF3hr_RNA//MA.png", height = 10, width = 20, units = 'in', res = 300, bg = "transparent")
ma_p
dev.off()
```
...
save.image("../output/CombinedUF3hr_RNA/CombinedUF3hrSalVAMPH_output.RData")
```
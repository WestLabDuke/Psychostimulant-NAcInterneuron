...
load("/media/west-lab-share/Melyssa_Minto/PV_SST/RNASeq/QCRDATA")
...
```{r, echo=F}
system(paste0("mkdir ", linux_path,"/Output_",format(Sys.time(), "PV3Hrtriplecheck2_QC_022021_%m%d%Y") ))
system(paste0("mkdir ", linux_path,"/Output_",format(Sys.time(), "PV3HrTtriplecheck2_QC_022021_%m%d%Y"), "/Figures" ))
pathFig = (paste0( linux_path,"/Output_",format(Sys.time(), "PV3Hrtriplecheck2_QC_022021_%m%d%Y"), "/Figures" ))
path = (paste0( linux_path,"/Output_",format(Sys.time(), "PV3Hrtriplecheck2_QC_022021_%m%d%Y") ))
```
...
# writing data
write_csv(data.frame(genes_filt, counts_filt), paste0(path, "/recheckGEOraw_counts_filt.csv"))
write_csv(data.frame(genes_filt, rpkm_filt), paste0(path, "/recheckGEOrpkm_counts_filt.csv"))
write_csv(data.frame(genes_filt, tpm_filt), paste0(path, "/recheckGEOtpm_counts_filt.csv"))
```
...
save.image(paste0(path, "/filedataSST35minSalVAMPHFigure2SUBMISSION.RData")) 
```
...
```{r, echo = F}
png(paste0(pathFig, "/Tree_vstCountss.png"), height = 7, width = 12, units = 'in', res = 300)
plot(fit)
dev.off()
...
```{r}
write_csv(summaryDE, paste0(path, "/test4colcpmsummaryDE.csv"))
write_csv(exp_mat, paste0(path, "/test4colfiltdiffExpressionMatrix.csv"))

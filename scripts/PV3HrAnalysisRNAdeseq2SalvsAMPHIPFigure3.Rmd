...
load("../data/PV3HrSalvAMPH_input.RData")
...
# writing data
write_csv(data.frame(genes_filt, counts_filt), "../output/PV3HrSalvAMPH_RNA/raw_counts_filt.csv")
write_csv(data.frame(genes_filt, rpkm_filt), "../output/PV3HrSalvAMPH_RNA/rpkm_counts_filt.csv")
write_csv(data.frame(genes_filt, tpm_filt),  "../output/PV3HrSalvAMPH_RNA/tpm_counts_filt.csv")
```
...
write.csv(exp_mat, "../output/PV3HrSalvAMPH_RNA/VarianceStabilizedCounts.csv")
head(exp_mat)
```
...
```{r}
png("../output/PV3HrSalvAMPH_RNA/Tree_vstCounts.png", height = 7, width = 12, units = 'in', res = 300)
plot(fit)
dev.off()

png("../output/PV3HrSalvAMPH_RNA/PCA.png", height = 7, width = 12, units = 'in', res = 300)
pcaPlt
dev.off()
```
...
```{r}
write_csv(exp_mat, "../output/PV3HrSalvAMPH_RNA/diffExpressionMatrix.csv")
write_csv(as.data.frame(amphIPvsalIP@listData), "../output/PV3HrSalvAMPH_RNA/amphIPvsalIP.csv")
```
...
png( "../output/PV3HrSalvAMPH_RNA/volcano.png",height = 10, width = 20, units = 'in', res = 300)
volcano_p
dev.off()

png( "../output/PV3HrSalvAMPH_RNA//MA.png", height = 10, width = 20, units = 'in', res = 300, bg = "transparent")
ma_p
dev.off()
```
...
save.image("../output/PV3HrSalvAMPH_RNA/PV3HrSalvAMPH_RNA_output.RData")
```
